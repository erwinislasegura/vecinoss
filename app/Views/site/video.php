...
<section class="video-after shell">
    <div class="video-description"><small>SOBRE ESTE VIDEO</small><?php if ($video['description']): ?><div><?= str_contains($video['description'], '<') ? $video['description'] : nl2br(e($video['description'])) ?></div><?php else: ?><p>Contenido audiovisual de VecinoSS TV.</p><?php endif; ?></div>
    <div class="share-bar compact-share" aria-label="Compartir este video"><span>Compartir</span><a href="https://www.facebook.com/sharer/sharer.php?u=<?= rawurlencode($shareUrl) ?>" target="_blank" rel="noopener" aria-label="Facebook"><img src="<?= asset('images/social/facebook.svg') ?>" alt=""></a><a href="https://twitter.com/intent/tweet?url=<?= rawurlencode($shareUrl) ?>&text=<?= rawurlencode($video['title']) ?>" target="_blank" rel="noopener" aria-label="X"><img src="<?= asset('images/social/x.svg') ?>" alt=""></a><a href="https://wa.me/?text=<?= rawurlencode($video['title'].' '.$shareUrl) ?>" target="_blank" rel="noopener" aria-label="WhatsApp"><img src="<?= asset('images/social/whatsapp.svg') ?>" alt=""></a><button type="button" data-copy-url="<?= e($shareUrl) ?>" aria-label="Copiar enlace"><img src="<?= asset('images/social/link.svg') ?>" alt=""><b>Copiar</b></button></div>
</section>
