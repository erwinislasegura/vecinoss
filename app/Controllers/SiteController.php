...
{
    public function home(): void { $posts=Post::published(40); $this->render('site/home',['title'=>'La voz de nuestra gente','featured'=>array_shift($posts),'posts'=>$posts,'categories'=>Category::topLevel(),'weather'=>Setting::weather(),'videos'=>Video::published()]); }
    ...
    public function category(string $slug): void { $category=Category::findBySlug($slug); if(!$category){http_response_code(404);$this->render('site/404',['title'=>'Categoría no encontrada']);return;} $this->render('site/category',['title'=>$category['name'],'category'=>$category,'posts'=>Post::published(24,(int)$category['id']),'categories'=>Category::topLevel()]); }
