...
final class Post
{
    ...
    public static function published(int $limit=12, ?int $categoryId=null): array { $sql=self::SELECT.' WHERE p.status="published" AND p.published_at<=NOW()'.($categoryId?' AND p.category_id=?':'').' ORDER BY p.featured DESC,p.published_at DESC LIMIT '.(int)$limit; $s=Database::connection()->prepare($sql); $s->execute($categoryId?[$categoryId]:[]); return $s->fetchAll(); }
    public static function featured(): ?array { $s=Database::connection()->query(self::SELECT.' WHERE p.status="published" AND p.published_at<=NOW() ORDER BY p.featured DESC,p.published_at DESC LIMIT 1'); return $s->fetch() ?: null; }
